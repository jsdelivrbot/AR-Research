...
class cvApp:
    ...
    def __init__(self, renderer):
        ...
        self.num_points = 0;
        self.mouse_vert_mat = np.array([8, 3], dtype='int32');
    ...
    def mouse_clicked(self, mouse_pos, vertex_pos):
        ...
        ## antes era cv.mSet ## no anda
        '''
        cv2.mSet(self.mouse_vert_mat, self.num_points*2, 0, mouse_pos.getX());
        cv2.mSet(self.mouse_vert_mat, self.num_points*2, 1, mouse_pos.getY());
        cv2.mSet(self.mouse_vert_mat, self.num_points*2, 2, 0);
        
        cv2.mSet(self.mouse_vert_mat, self.num_points*2 + 1, 0, vertex_pos.getX());
        cv2.mSet(self.mouse_vert_mat, self.num_points*2 + 1, 1, vertex_pos.getY());
        cv2.mSet(self.mouse_vert_mat, self.num_points*2 + 1, 2, vertex_pos.getZ());
        '''
    ...
    def pull_frame(self):
        ...
        """
        if (self.image_idx == 1):
            cvim = cv2.imread("../data/20141020_214650.jpg", cv2.IMREAD_GRAYSCALE);
        elif (self.image_idx == 2):
            cvim = cv2.imread("../data/20141020_214655.jpg", cv2.IMREAD_GRAYSCALE);
        else:
